...
describe("Consumer", () => {
  ...
  describe("Options Validation", () => {
    ...
    it("requires a queueId to be set", () => {
      assert.throws(() => new Consumer({ accountId: ACCOUNT_ID }), {
    ...
    it("require a handleMessage or handleMessageBatch function to be set", () => {
      assert.throws(
        () => new Consumer({ accountId: ACCOUNT_ID, queueId: QUEUE_ID }),
        {
          message:
  ...
  describe(".start", () => {
    ...
    it("deletes the message when the handleMessage function is called", async () => {
      ...
      sandbox.assert.calledWithMatch(fetch, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          authorization: `Bearer ${QUEUES_API_TOKEN}`,
        },
        body: JSON.stringify({ ackIds: ["123"] }),
      });
    });
    ...
    it("does not delete the message if shouldDeleteMessages is false", async () => {
      ...
      sandbox.assert.neverCalledWithMatch(fetch, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          authorization: `Bearer ${QUEUES_API_TOKEN}`,
    ...
    it("doesn't consume more messages when called multiple times", () => {
      fetch
        .withArgs(PULL_MESSAGES_ENDPOINT, fetchArgs)
      ...
      fetch.calledOnceWith(PULL_MESSAGES_ENDPOINT, fetchArgs);
    });
    ...
    it("terminate message visibility timeout on processing error", async () => {
      ...
      sandbox.assert.calledWith(fetch.secondCall, mockChangeMessageVisibility);
      sandbox.assert.match(
